...
public class OrderHotelImpl implements OrderHotelDao {
    ...
    public List<OrderHotel> findByUserId(String userId, int currentPage, int pageSize) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                oh = new OrderHotel(userid, people, state,hotelName);
                list.add(oh);
            }
    ...
    public List<OrderHotel> findByHotelName(int currentPage, int pageSize, String hotelName) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                oh = new OrderHotel(userid, people, state,hotelName1);
                list.add(oh);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void update(String userId, String userId1, int people, int state,String hotelName) {
        String sql="update orderhotel set userId = ?,people = ?,state = ?,hotelName=? where userId = ?";
        try {
            conn = DBConn.getConnection();
            pre = conn.prepareStatement(sql);
