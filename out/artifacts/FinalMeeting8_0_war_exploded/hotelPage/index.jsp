...
<head>
    ...
    <style type="text/css">
        .table>thead>tr>th,td{text-align: center;}
        .main table{width: 90%;margin: 0px auto}
    </style>
    <script>
        $(function(){
        ...
        function disptime(){
            ...
            document.getElementById("myclock").innerHTML="<h1 style='font-size:20px '>"+hh+":"+mm+":"+ss+time+"</h1>";
        }
    ...
    <script language="JavaScript">
        function showNotice(){
            var listHtml="<div id=\"juzhong\">\n" +
                "\t\t\t\t\t<h1 align=\"center\">欢迎使用会议管理系统V1.0</h1><br>\t\t\t\t\n" +
                "\t\t\t\t\t<p align=\"center\" id=\"myclock\" style='color: green;font-weight: bolder'></p><br><br><br>\n" +
        ...
        function showOrder(num){
            ...
            xmlHttp.onreadystatechange = function (){
                if(xmlHttp.readyState==4){
                    ...
                    var listHtml = "<table class=\"table table-bordered table-striped table-hover\">\n" +
                    ...
                    if(num==1){
                        for(var i=0;i<obj.length;i++){
                            var k=i+1;
                            if(obj[i].state==0){
                                listHtml+="<tr class=\"warning\"><td>"+k+"</td><td>"+obj[i].userId+"</td><td>"+obj[i].people+"</td><td>"+obj[i].hotelName+"</td>" +
                                    "<td>未审核</td><td>"+obj[i].ordertime+"</td><td><button type=\"button\" class=\"btn btn-info\" onclick=\"examine('"+obj[i].userId+"','"+obj[i].ordertime+"')\"><span class=\"glyphicon glyphicon-pencil\"></span>&nbsp;审核</button></td></tr>";
                            }
                            if(obj[i].state==1){
                                listHtml+="<tr class=\"danger\"><td>"+k+"</td><td>"+obj[i].userId+"</td><td>"+obj[i].people+"</td><td>"+obj[i].hotelName+"</td>" +
                                    "<td>未通过</td><td>"+obj[i].ordertime+"</td><td><button type=\"button\" class=\"btn btn-danger\" onclick=\"deleteOrder('"+obj[i].userId+"','"+obj[i].ordertime+"')\"><span class=\"glyphicon glyphicon-trash\"></span>&nbsp;删除</button></td></tr>";
                            }
                            if(obj[i].state==2){
                                listHtml+="<tr class=\"success\"><td>"+k+"</td><td>"+obj[i].userId+"</td><td>"+obj[i].people+"</td><td>"+obj[i].hotelName+"</td>" +
                                    "<td>已通过</td><td>"+obj[i].ordertime+"</td><td><button type=\"button\" class=\"btn btn-warning\" onclick=\"cancel('"+obj[i].userId+"','"+obj[i].ordertime+"')\"><span class=\"glyphicon glyphicon-remove\"></span>&nbsp;取消</button></td></tr>";
                            }
                        }
                    }else if(num==2){
                        for(var i=0;i<obj.length;i++){
...
<body onload="initAJAX();showNotice();disptime()">
<div class="top"></div>
<div id="header">
    <div class="logo">酒店订单处理</div>
    <div class="navigation">
        <ul>
            <li>欢迎您！</li>
            <li><%=session.getAttribute("hotelID")%></li>
            <li><a id="changeInfo" style="cursor: pointer">修改信息</a></li>
            <li><a id="changePassword" style="cursor: pointer">修改密码</a></li>
            <li><a id="SignOut" style="cursor: pointer">退出</a></li>
        </ul>
    </div>
</div>
<div id="content">
    <div class="left_menu">
        <ul id="nav_dot">
            ...
            <li>
                <h4 class="M2"><span></span>查看订单</h4>
                <div class="list-item none">
                    <a href="javascript:showOrder(1) ">订单总览</a>
                    <a href="javascript:showOrder(2)">未审核订单</a>
                    <a href="javascript:showOrder(3)">未通过订单</a>
                    <a href="javascript:showOrder(4)">已通过订单</a>
                </div>
            </li>
        </ul>
    </div>
